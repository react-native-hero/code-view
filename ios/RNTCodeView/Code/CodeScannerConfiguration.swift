...
let bundlePath = Bundle(for: CodeScannerConfiguration.self).path(forResource: "CodeView", ofType: "bundle")!

let bundle = Bundle(path: bundlePath)

let budle1 = Bundle(for: CodeScannerConfiguration.self).resourceURL?.appendingPathComponent("/CodeView.bundle")
...
@objc public class CodeScannerConfiguration: NSObject {
    ...
    public var torchOnImage = UIImage(named: "code_scanner_torch_on", in: bundle, compatibleWith: nil)
    
    public var torchOffImage = UIImage(named: "code_scanner_torch_off", in: bundle, compatibleWith: nil)
